...
func main() {
    ...
    // :: oznacza „wszystkie interfejsy” w IPv6.

    // 1. Tworzymy socket UDP
    conn, err := net.ListenUDP("udp6", &addr)
    if err != nil {
        log.Fatalf("[UDP IPv6: Serwer]: Błąd ListenUDP: %v", err)
    }
    defer conn.Close()

    fmt.Println("[UDP IPv6: Serwer]: Serwer UDP IPv6 działa na porcie 9090...")
    ...
    go func() {
        <-stop
        fmt.Println("\n[UDP IPv6: Serwer]: Otrzymano sygnał zakończenia. Zamykam socket UDP...")
        conn.Close()
        os.Exit(0)
    }()
    // ============================================================
    ...
    for {
        ...
        if err != nil {
            log.Printf("[UDP IPv6: Serwer]: Błąd ReadFromUDP: %v", err)
            continue
        }
        ...
        if !knownClients[addrStr] {
            fmt.Printf("[UDP IPv6: Serwer]: Nowy klient: %s\n", addrStr)
            knownClients[addrStr] = true
        }

        msg := string(buf[:n])
        fmt.Printf("[UDP IPv6: Serwer]: Odebrano od %s: %s\n", remoteAddr, msg)

        // 3. Odsyłamy odpowiedź
        conn.WriteToUDP([]byte("[UDP IPv6: Serwer]: Serwer odsyła odpowiedź: "+msg), remoteAddr)
    }
}
