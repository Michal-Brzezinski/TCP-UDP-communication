...
func main() {
    ...
    // [::] to adres IPv6 oznaczający „wszystkie interfejsy” (odpowiednik 0.0.0.0 w IPv4)

    if err != nil {
        log.Fatalf("[TCP IPv6: Serwer]: Błąd Listen: %v", err)
    }
    fmt.Println("[TCP IPv6: Serwer]: Serwer TCP IPv6 działa na porcie 8080...")
    ...
    go func() {
        <-stop
        fmt.Println("\n[TCP IPv6: Serwer]: Otrzymano sygnał zakończenia. Zamykam listener...")
    ...
    for {
        ...
        if err != nil {
            log.Printf("[TCP IPv6: Serwer]: Błąd Accept: %v", err)
            continue
        }
...
func handleClient(conn net.Conn) {
    ...
    fmt.Printf("[TCP IPv6: Serwer]: Połączono z %s\n", conn.RemoteAddr())
    ...
    for reader.Scan() {
        ...
        */
        ...
        fmt.Printf("[TCP IPv6: Serwer]: Odebrano od %s: %s\n", conn.RemoteAddr(), msg)
        
        // 4. Echo — odsyłamy to samo
        conn.Write([]byte("[TCP IPv6: Serwer odsyła odebraną wiadomość]: " + msg + "\n"))
    ...
    if err := reader.Err(); err != nil {
        fmt.Printf("[TCP IPv6: Serwer]: Błąd przy czytaniu od %s: %v\n", conn.RemoteAddr(), err)
    }

    fmt.Printf("[TCP IPv6: Serwer]: Rozłączono: %s\n", conn.RemoteAddr())
