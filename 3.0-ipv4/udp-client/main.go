...
func main() {
    ...
    if err != nil {
        log.Fatalf("[UDP IPv4: Klient]: Błąd DialUDP: %v", err)
    }
    defer conn.Close()

    fmt.Println("[UDP IPv4: Klient]: Połączono z serwerem UDP.")

    // ============================================================
    //  Wypisanie portu źródłowego klienta UDP
    // ============================================================
    fmt.Printf("[UDP IPv4: Klient]: Mój adres lokalny (IP:port): %s\n", conn.LocalAddr())
    ...
    for {
        ...
        if text == "quit" {
            fmt.Println("[UDP IPv4: Klient]: Zamykanie klienta UDP.")
            break
        }
        ...
        if err != nil {
            fmt.Printf("[UDP IPv4: Klient]: Błąd przy wysyłaniu: %v\n", err)
            break
        }
        ...
        if err != nil {
            fmt.Printf("[UDP IPv4: Klient]: Błąd przy odbiorze odpowiedzi: %v\n", err)
            break
        }

        fmt.Printf("[UDP IPv4: Klient - Odpowiedź serwera]: %s\n", string(buf[:n]))
    }

    fmt.Println("[UDP IPv4: Klient]: Połączenie zakończone.")
}
